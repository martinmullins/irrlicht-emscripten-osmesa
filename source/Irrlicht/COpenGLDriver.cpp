	...
	SDLDevice(device), DeviceType(EIDT_SDL)
{
  ...
  #ifdef OSMESA
  MesaContext = OSMesaCreateContextExt( OSMESA_RGBA, 16, 0, 0, NULL );
  MesaBuffer = malloc(4 * Params.WindowSize.Width *  Params.WindowSize.Width * sizeof(GLubyte));
  if (!OSMesaMakeCurrent( MesaContext, MesaBuffer, GL_UNSIGNED_BYTE, Params.WindowSize.Width,  Params.WindowSize.Width)) {
    printf("Failed to create initial context\n");
    abort();
  }
  #endif
...
// TODO looks like there is probably an emscripten canvas API to use here
#ifdef __EMSCRIPTEN__
EM_JS(void, write_canvas, (const void* buffer), {
